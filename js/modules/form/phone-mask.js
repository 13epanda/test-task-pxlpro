...
const initJsPhoneMask = () => {
  const phoneJsInputs = document.querySelectorAll('.js-phone');
  if (phoneJsInputs.length) {
    phoneJsInputs.forEach((input) => {
      input.addEventListener('focus', onFocusPhoneInput);
      if (input.value) {
        replacePhoneValue(input);
        input.addEventListener('input', onInputPhoneInput);
...
window.initPhoneClassMask = () => {
  initJsPhoneMask();
};

export {initPhoneMask, initJsPhoneMask};
